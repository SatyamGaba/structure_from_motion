...
from inverse_warp import pose_vec2mat, inverse_warp_posemat
...
parser.add_argument('-f', '--training-output-freq', type=int,
                    ...
                    metavar='N', default=0)
parser.add_argument('--with-photocon-loss', action='store_true', dest='with_photocon_loss', help='Adds additional photometric consistency loss for t-1 and t+1')
...
#             print("----",T13.size())
            # target = 1 and ref = 3
            ref_img_warped, valid_points = inverse_warp_posemat(ref_imgs[1], depth[0][:,0], T13,
                                                        intrinsics,
                                                        args.rotation_mode, args.padding_mode)
...
#             print("----",T13.size())
            ...
            ref_img_warped, valid_points = inverse_warp_posemat(ref_imgs[1], depth[:,0], T13,
                                                        intrinsics,
                                                        args.rotation_mode, args.padding_mode)
            diff = (ref_imgs[0] - ref_img_warped) * valid_points.unsqueeze(1).float()
            loss_4 = diff.abs().mean()
