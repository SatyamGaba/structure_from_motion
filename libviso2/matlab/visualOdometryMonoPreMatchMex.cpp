...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  if (!strcmp(command,"init")) {
    ...
    for (int32_t i=0; i<mxGetNumberOfFields(prhs[1]); i++) {
      ...
      if (mxIsDouble(field_val)) {
        ...
        if (!strcmp(field_name,"pre_step_size") )         param.match.pre_step_size = val;
  ...
  // process
    ...
    if (nrhs!= 2 )
      mexErrMsgTxt("1 inputs required: matches.");
    if (nlhs!=1 )
    ...
    if (nrhs==2+1 && !mxIsSingle(prhs[1]) )
      mexErrMsgTxt("preMatches must be a double scalar (0/1).");
    
    // get image pointers
    float* matches = (float *)mxGetPr(prhs[1]);
    const size_t *dims_match = mxGetDimensions(prhs[1] );
    
    int32_t  dims_match_[] = {dims_match[1], dims_match[0] };
        
    // compute visual odometry, return motion estimate on success
    // (mapping from previous to current frame)
    if (viso -> process(matches, dims_match_) ) {
