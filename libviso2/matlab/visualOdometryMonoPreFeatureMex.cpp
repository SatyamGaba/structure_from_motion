...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  // process
    ...
    if (nrhs!=2+1 && nrhs!=3+1)
      mexErrMsgTxt("2 or 3 inputs required: I (image), f (feature), [replace].");
    if (nlhs!=1) 
    ...
    float *        f    = (float *)mxGetPr(prhs[2]);
    const size_t *dims =   mxGetDimensions(prhs[1]);
    const size_t *dims_feature = mxGetDimensions(prhs[2] );
